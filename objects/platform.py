...
import random, pygame, uuid, Constants
...
class Platform:
    ...
    def __init__(self, yMin, yMax, gms, gmp):
        ...
        self.fast = (
            Constants.screenWidth / 4 / Constants.FPS
        )  # taille de l'ecran /  temps a l'ecran en s / nb de frames par secondes
        self.img = pygame.image.load(
            "./drawable/" + self.gameScene.getCurrentTheme().platform
        ).convert_alpha()
    ...
    def destroy(self):
        destroyEvent = pygame.event.Event(
            Constants.PLATFORM_DELETE_EVENT,
            {
                "id": self.id,
                "x": self.x,
                "y": self.y,
