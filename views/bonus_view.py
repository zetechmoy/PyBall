...
import random, pygame, Constants, components.helper as helper, components.colors as colors
...
class BonusView:
    def __init__(self, gamescene, gamepanel):
        ...
        self.img = pygame.image.load(
            "./drawable/" + self.gameScene.getCurrentTheme().bonus
        ).convert_alpha()
        ...
        ):  # NE PAS FAIRE UN BONUS A CHAQUE COLLISION => random.randint(0, 4) == 2
            self.isABonus = True
    ...
    def draw(self, fenetre):
        if self.coinGiven == False:
            if self.isABonus == True:
                fenetre.blit(
                    self.shadow, (self.x + self.width / 12, self.y + self.height / 12)
                )
                rect = helper.surfaceFromRect(
                    pygame.Rect(self.x, self.y, self.width, self.height),
                    self.gameScene.getCurrentTheme().ballColor,
                )
