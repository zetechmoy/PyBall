...
class BallView():
    def __init__(self, gmp, ball, x, y):
        ...
        self.width, self.height =  self.gamePanel.screenWidth/2, self.gamePanel.screenHeight/4

        self.ballHeight = self.ballWidth = int((self.height - 4*self.space)/2)

        self.backgroundRect = pygame.Rect(0, 0, int(self.width-2*self.space), int(self.height-2*self.space))
        self.backgroundSurface = Helper.surfaceFromRect(self.backgroundRect, Colors.deep_purple)
        self.backgroundPos = (x+self.space, y+self.space)

        self.oriX = x+self.space
        self.oriY = y+self.space

        self.ballSpritesheet = pygame.image.load('./drawable/'+self.ball.yellow_spritesheet)
        self.ballPreview = self.image_at(pygame.Rect(0, 0, 100, 100))
        self.ballPreview = pygame.transform.scale(self.ballPreview, (self.ballWidth, self.ballHeight))
        self.ballPos = (self.backgroundPos[0] + self.width/4 - self.ballWidth/2,
            self.backgroundPos[1] + self.height/2 - self.ballHeight/2 - self.space)

        self.shadow = pygame.image.load('./drawable/ball_shadow.png')
        self.shadow = pygame.transform.scale(self.shadow, (self.ballWidth+2*self.space, self.ballHeight+2*self.space))
        self.shadowPos = (self.backgroundPos[0] + self.width/4 - (self.ballWidth+2*self.space)/2,
            self.backgroundPos[1] + self.height/2 - (self.ballHeight+2*self.space)/2 - self.space)
        ...
        self.ballTitlePos.y = self.backgroundPos[1] + self.height/2 - self.ballTitlePos[3]
        ...
        self.selectorPos = (int(self.backgroundPos[0] + (self.width/4*3)),
    ...
    def draw(self, fen):
        ...
        self.selectorRect = pygame.draw.circle(fen, Colors.yellow, self.selectorPos, self.selectorRadius)

        if(self.paid == True):
            if(self.selected == True):
                pygame.draw.circle(fen, Colors.green, self.selectorPos, int(self.selectorRadius/2))
            else:
                pygame.draw.circle(fen, Colors.yellow, self.selectorPos, int(self.selectorRadius/2))
        else:
            pygame.draw.circle(fen, Colors.red, self.selectorPos, int(self.selectorRadius/2))
    ...
    def getSelectorPos(self):
        ...
        return tuple(map(operator.sub, self.selectorPos, (self.selectorRadius, self.selectorRadius)))